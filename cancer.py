...
label=Label(root,text="Breast Cancer Prediction",font=('bold',20),fg="red")
...
def predict():
	...
	texture=float(tex.get())
	marea=float(area.get())
	perimeter=float(per.get())
	smoothness=float(smooth.get())
	nb=GaussianNB()
	nb.fit(x_train,y_train)
	predn=nb.predict(x_test)
	newp=nb.predict([[radius,texture,marea,perimeter,smoothness]])
	label=Label(root,text="person cancer status: "+str(newp),font=('bold',16),fg='red')
	label.grid(row=11,column=2,padx=10,pady=10)

rad=Entry(root,width=27,bg="orange")
rad.grid(row=9,column=0,padx=10,pady=10)
rad.insert(0,'mean_radius')
tex=Entry(root,width=27,bg="orange")
tex.grid(row=9,column=1,padx=10,pady=10)
tex.insert(0,'mean_texture')
area=Entry(root,width=27,bg="orange")
area.grid(row=9,column=2,padx=10,pady=10)
area.insert(0,'mean_area')
per=Entry(root,width=27,bg="orange")
per.grid(row=9,column=3,padx=10,pady=10)
per.insert(0,'mean_perimeter')
smooth=Entry(root,width=27,bg="orange")
smooth.grid(row=9,column=4,padx=10,pady=10)
smooth.insert(0,'mean_smoothness')
